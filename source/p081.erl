...
%% NOT WORKING
%% This is basically a bi-tree, and we can solve this by using a bredth first algorithm

solve() ->
	{ok, Bin} = file:read_file("data/p081_matrix.txt"),
	...
	%	 ,805,732,524,37,331
	%	],
	Next = triangle_iterator(),
	Len = round(math:sqrt(length(M))),
	M1 = to_tri(lists:reverse(M), Len),
	Len1 = length(M1)+1,
	%io:format("M1: ~p~n~n", [M1]),
	io:format("Len: ~p~n~n", [Len]),
	io:format("Len1: ~p~n~n", [Len1]),
	ok.
